...
The platform is a monolithic Laravel application with separate API endpoints for mobile clients and an administrative dashboard. The system integrates live video coaching through Agora SDK, session scheduling with availability management, in-app chat, and payment processing. It functions as a standalone platform managing the entire workflow from provider registration and certification review through session completion and billing. Alongside one-to-one bookings, the platform hosts group events and workshops with a named speaker that clients can register interest in.

## My Technical Role

I was responsible for the complete backend development including RESTful API design and implementation, database schema design for booking and scheduling system, real-time video session management with Agora integration, availability and time slot management system, authentication and authorization with role-based access control, push notification system with multilingual support, payment processing integration with Stripe, certification and document verification workflow for provider onboarding, chat messaging system for pre and post-session communication, and webhook handling for third-party service integrations.

## Backend Architecture Highlights

The API follows RESTful conventions with resource-based endpoints organized by domain functionality (bookings, sessions, availabilities, slots, specialties, profiles, chat). Users are modelled polymorphically: a single `users` table points to either a `Provider` or a `Client` through a `userable` morph relation, so authentication and profile data stay unified while role-specific data lives in its own table. Authentication is implemented using Laravel Sanctum with token-based access control and ability-based permissions that distinguish between providers (with approval status), clients, and administrators. The system implements separate authentication guards for API users and admin panel access.

Data flow is request-driven with Eloquent ORM handling database interactions. Live video coaching sessions are managed through Agora SDK integration with dynamic token generation for secure room access, issued per participant and stored on the session-client pivot. The architecture implements a polymorphic relationship system where sessions can be associated with different bookable items (bookings, events), providing flexibility for one-to-one coaching and group formats alike. Availability management uses recurring schedule patterns stored at the provider level, with time slots dynamically generated and managed at the session level.

Key architectural decisions include a polymorphic session model that allows sessions to be linked to bookings or standalone events, a many-to-many session-participant relation that supports group training as well as one-to-one coaching, slot-based scheduling system where providers define availability patterns and clients book specific time slots, webhook-based Agora event logging for session tracking and analytics, certification verification workflow with admin approval gates, and an event-driven notification system where booking status changes trigger push notifications automatically.

## Database & Data Handling

The system uses MySQL/MariaDB as the relational database with Eloquent ORM for object-relational mapping. The schema design emphasizes relationship modeling with separate tables for providers, clients, bookings, sessions, slots, specialties, and availabilities. Sessions attach polymorphically to whatever was booked, while slots are managed separately to handle time-specific availability. Provider-specialty is a many-to-many relation carrying the price on the pivot, so the same coach can charge differently per specialty.

Bookings also act as the client intake record, storing the training-relevant fields the provider needs before the first session — weight, height, allergies, previous injuries — plus free-text notes and file attachments.
...
**Certification Verification Workflow**: Coaching and nutrition advice carries real liability, so a provider cannot take paying clients before their certifications are checked. Providers upload their certificates and identity documents, which open a verification request that an admin must approve. This created complexity in permission management: providers needed access to profile and media management immediately after signup, but had to stay blocked from accepting bookings until verified. This was solved through ability-based tokens with intermediate states (`provider-notApproved`, `provider-full`), allowing conditional feature access based on verification status without branching logic scattered through the controllers.

**Payment Capture Timing**: A client should not be charged for a session a coach never accepts. Managing payment processing where funds are captured only after provider approval or session completion required implementing a hold-and-capture mechanism on Stripe payment intents: the intent is authorized when the booking is created and captured only on approval, while a declined or canceled booking cancels the intent and releases the hold instead of triggering a manual refund. Invoices carry the Stripe session and payment-intent identifiers along with their own status lifecycle (pending, authorized, paid, expired), so the financial record stays reconcilable against Stripe.

**Natural-Language Provider Search**: Beyond filtering by specialty and city, clients can describe what they are looking for in plain language ("someone who trains beginners for a first marathon"). The `/search/ai` endpoint passes the prompt to a GPT-backed recommendation chat that returns candidate provider IDs, which are then re-filtered through the normal authorization scopes so only active, verified, approved providers are ever returned. The model's output is treated as a ranking hint, never as an access decision — a malformed or hallucinated response degrades to the standard listing rather than leaking unapproved providers.
...
The platform lets independent coaches and dietitians build a client base beyond their local gym or clinic, and gives clients access to specialists they could not reach in person. The automated scheduling system removes the back-and-forth of arranging sessions over chat by handling availability, booking, and client communication in one place. Integrated video means neither side needs an external meeting tool, and the intake data captured at booking time means the coach arrives at the session already knowing the client's measurements, allergies, and injury history.

The certification verification workflow protects clients by vetting providers before they can accept paid bookings — an important trust signal in a market where anyone can claim to be a trainer. The flexible scheduling model accommodates everything from a single session to recurring weekly training blocks, and the group-session and events support extends the same infrastructure to workshops. The notification system keeps both sides informed of booking status changes, reducing no-shows.
